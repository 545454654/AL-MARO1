...
export default function App() {
  ...
  // ==========================================
  // PREDICTION GENERATOR & FIREBASE UTILS
  // ==========================================
  const mapFirebaseData = (rawData: any) => {
    ...
    for (let i = 1; i <= 50; i++) {
      ...
      if (value !== undefined) {
        if (typeof value === 'object' && value !== null) {
          ...
          formatted[key] = { [key]: String(value) };
        }
      }
    }
    return formatted;
  ...
  const fetchFirebasePredictions = async () => {
    try {
      const dbRef = ref(rtdb, 'm11');
      const snapshot = await get(dbRef);
      if (snapshot.exists()) {
        const rawData = snapshot.val();
        const mapped = mapFirebaseData(rawData);
        if (mapped && Object.keys(mapped).length > 0) {
          return mapped;
        }
      }
    } catch (error) {
      console.error("Firebase fetch failed, falling back to local simulation:", error);
    }
    return null;
  };
  ...
  const generatePredictions = async () => {
    ...
    // Map 10 rows (from 0 to 9)
    for (let r = 0; r < 10; r++) {
      // Determine safe counts based on row index
      let safeCount = 4;
      if (r >= 4 && r < 7) safeCount = 3;      // Rows 4, 5, 6
      if (r >= 7 && r < 9) safeCount = 2;      // Rows 7, 8
      if (r >= 9) safeCount = 1;               // Top Row 9

      // Choose unique safe columns randomly among the 5 columns (0 to 4)
      ...
      // Fill in all columns (0 to 4) using the magic index formula
      for (let c = 0; c < 5; c++) {
        const mIndex = r * 5 + c + 1; // Magic formula: r * 5 + c + 1
  ...
  const isSafeApple = (rowIdx: number, colIdx: number, currentPreds = predictions) => {
    if (!currentPreds || Object.keys(currentPreds).length === 0) return false;
    
    // 1. Calculate magic index
    const mIndex = rowIdx * 5 + colIdx + 1;
    const mKey = `m${mIndex}`;
    
    // 2. Read corresponding nested object
    const mObj = currentPreds[mKey];
    
    // 3. Verify it is equal to "1"
    if (mObj && typeof mObj === 'object' && mObj[mKey] === "1") {
      return true; // Apple is safe!
    }
    return false; // Poison/bomb apple
  };
  ...
  const showPlatformConnectionDialog = (platformName: string) => {
    Swal.fire({
      ...
      html: `⏳ <b>${platformName}</b> <br> جاري التوصيل بالخادم وتهيئة مسار البيانات...`,
      timer: 2000,
  ...
  const handleGuessPrediction = async () => {
    ...
    Swal.fire({
      ...
      html: '⏳ جاري جلب مفاتيح التوزيع وقراءة الملف التنبؤي من Firebase...',
      timer: 1600,
      timerProgressBar: true,
      showConfirmButton: false,
      allowOutsideClick: false,
    ...
    // Try fetching from Firebase with a fallback
    let fetchedPreds = await fetchFirebasePredictions();
    let isFromFirebase = true;
    if (!fetchedPreds) {
      fetchedPreds = await generatePredictions();
    ...
    setTimeout(() => {
      ...
      Swal.fire({
        ...
        html: isFromFirebase 
          ? `✅ تم جلب وقراءة التوقعات بنجاح من قاعدة البيانات Realtime Database.`
          : `✅ تم توليد وتخزين التوقعات بنجاح في قاعدة البيانات المؤقتة.`,
        icon: 'success',
        timer: 1200,
        showConfirmButton: false,
        customClass: {
    ...
    >
          ...
          {!isLoggedIn ? (
            ...
            >
              ...
              <p className="text-xs text-center text-gray-400 mb-8 max-w-xs leading-relaxed font-light">
                نظام كشف المسارات المتقدم للعبة تفاحة الحظ 🍎
              </p>
      ...
      {isProcessing && (
        <div className="fixed inset-0 bg-black/75 z-40 flex flex-col items-center justify-center gap-4 backdrop-blur-sm">
          ...
          <span className="text-lg font-bold text-red-500 animate-pulse tracking-widest">جاري التخمين وتحميل المسار التنبؤي...</span>
        </div>
      )}
